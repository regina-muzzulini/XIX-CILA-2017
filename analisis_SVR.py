    ...
    for tramoTrain in np.unique(data[:, 0]):  
      if tramoTrain==21:
        ...
        if anioMax >= 2:
            ...
            train_lbl_draw = list()
            yPredict_SVR_draw = list()
            test_data_error = list()
            test_data_lbl_error = list()
            yPredict_SVR_error = list()
            ...
            while indice <= indice_maximo_tramo-3:
                ...
                yPredict_SVR_draw.append(data_lbl[indice])				
                indice += 1
            ...
            while indice <= indice_maximo_tramo:
                if firts:
                    ...
                    test_data_error.append(data[indice, 1:])
                    
                    train_data = preprocessing.scale(train_data)
                else:
                    yPredict_SVR2 = preprocessing.scale(yPredict_SVR)
                    train_data_lbl=list(train_data_lbl)
                    train_data_lbl.append(yPredict_SVR[0])
                    train_data=list(train_data)
                    ...
                    aux_iri = yPredict_SVR[0]		#test_data[0, 5]
                    train_data.append([anio_aux, aux_defle, aux_tl, aux_tm, aux_tp, aux_iri])
					
                    test_data_lbl = list()
                    test_data = list()
					
                    test_data.append([data[indice, 1],data[indice, 2],data[indice, 3],data[indice, 4],data[indice, 5],yPredict_SVR2[0]])
                    test_data_error.append([data[indice, 1],data[indice, 2],data[indice, 3],data[indice, 4],data[indice, 5],yPredict_SVR2[0]])
                ...
                test_data_lbl_error.append(data_lbl[indice])
                #train_lbl_draw.append(data_lbl[indice])
                train_lbl_draw.append(dataOriginal_lbl[indice])	
                ...
                clf = SVR(C=0.1, kernel='rbf', epsilon=0.1)
                clf.fit(train_data, train_data_lbl)

                yPredict_SVR = clf.predict(test_data)
                yPredict_SVR_draw.append(yPredict_SVR[0])
                yPredict_SVR_error.append(yPredict_SVR[0])
            ...
            while anioPred <= 3:
                ...
                while i < size_test:
                    ...
                    test_data[i, 5] = yPredict_SVR2[i]
		    
                    train_data_lbl=list(train_data_lbl)
                    train_data_lbl.append(yPredict_SVR[i])
            ...
            print(tramoTrain)
            #print(yPredict_SVR_draw)

            if args.plot:
                #Graficamos
                data_to_plot_dict = {"gt": train_lbl_draw, "svr": yPredict_SVR_draw}
                plotearContinuo_SVR.plot_personalizado(data_dict=data_to_plot_dict,
